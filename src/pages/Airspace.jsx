  ...
  return (
    <div>
      <section className="mb-8 rounded-3xl bg-slate-900/70 border border-white/10 p-5 md:p-7 shadow-card relative overflow-hidden">
        <div className="absolute -top-20 -right-20 w-72 h-72 bg-sky-500/10 rounded-full blur-3xl"></div>

        <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div>
            <p className="text-sky-300 text-sm font-bold mb-2 flex items-center gap-2">
              <ScanLine size={16} />
              Airspace Operations
            </p>
            ...
            <p className="text-slate-400 mt-3 max-w-3xl">
              Track aircraft, drones, restricted-zone activity, aerial risk
              levels and surveillance status using live Supabase records.
            </p>
          </div>
      ...
      <div className="grid md:grid-cols-4 gap-5 mb-8">
        <Card icon={<Plane />} title="Aircraft Tracked" value={aircraftCount} color="text-sky-300" />
        <Card icon={<Radar />} title="Radar Tracks" value={radarCount} color="text-emerald-300" />
        <Card icon={<AlertTriangle />} title="Threat Alerts" value={alertCount} color="text-red-300" />
        <Card icon={<Radio />} title="Drone Signals" value={droneCount} color="text-yellow-300" />
      </div>
      ...
      {canCreate && (
        <div className="bg-slate-900/80 border border-white/10 rounded-3xl p-5 md:p-6 mb-8 shadow-card">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <FormInput value={aircraftName} onChange={setAircraftName} placeholder="Aircraft or drone name" />
            <FormInput value={location} onChange={setLocation} placeholder="Location e.g. Restricted Zone" />
            <Select value={assetType} onChange={setAssetType} options={["Aircraft", "Drone", "Helicopter", "Unknown"]} />
